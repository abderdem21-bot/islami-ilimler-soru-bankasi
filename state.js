...
const USERS_STORAGE_KEY = 'islami_app_users';
...
window.firebaseRegister = function(email, password) {
  return new Promise((resolve, reject) => {
    const users = getUsers();
    if (users[email]) {
...
window.firebaseResetPassword = function(email) {
  return new Promise((resolve, reject) => {
    ...
    saveUsers(users);
    resolve({ email, newPassword });
  });
};

// ========== BAŞLANGIÇ ADMIN KULLANICISI ==========
(function initUsers() {
  const users = getUsers();
  if (!users["abderdem21@gmail.com"]) {
    users["abderdem21@gmail.com"] = {
      password: "123456",
      uid: "c6HreG568dennHzBCXwpUs6QvM23",
      isAdmin: true
    };
    saveUsers(users);
  }
